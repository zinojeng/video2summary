...
class AudioTranscriber:
    ...
    MAX_FILE_SIZE = 5 * 1024 * 1024  # 5 MB (Clawbot 環境限制)
    ...
    def transcribe_file(
        ...
        if "gemini" in model.lower():
            ...
            # 新 SDK 直接吃 `gemini-x`，不需要舊 SDK 的 `models/` 前綴
    ...
    def translate_text(self, text, target_lang, model=OPENAI_TRANSLATION):
        """翻譯文字"""
    ...
    def translate_segments_batch(self, segments, target_lang, model=OPENAI_TRANSLATION):
        """
...
def main():
    ...
    parser.add_argument("--max-segment-seconds", type=int, default=480,
                       help="分段長度（秒），預設 480 秒 (8 分鐘，適合 5MB 上限)")
